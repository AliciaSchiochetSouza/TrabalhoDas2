...
def extract_cliente(myTimer: func.TimerRequest) -> None:
    ...
    logging.info(f"Iniciando integração de Clientes. Lendo de: {sql_database_source} | Gravando em: {sql_database_dest}")
    ...
    try:
        ...
        with pyodbc.connect(conn_str_source) as conn_source:
            ...
            if not rows:
                logging.warning("Nenhum registro encontrado na origem (erp.cliente).")
                return

            columns = [column[0] for column in cursor_source.description]
            logging.info(f"Extração de clientes bem-sucedida: {len(rows)} registros encontrados.")
        ...
        with pyodbc.connect(conn_str_dest) as conn_dest:
            ...
            logging.info(f"Carga finalizada: {len(rows)} clientes inseridos com sucesso no destino.")          

    except pyodbc.Error as e:
        logging.error(f"Erro de SQL no processamento de clientes: {str(e)}")
        raise
    except Exception as e:
        logging.error(f"Erro inesperado no processo de clientes: {str(e)}")
        raise