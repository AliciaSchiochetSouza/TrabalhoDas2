...
def extract_estoque_movimentacao(myTimer: func.TimerRequest) -> None:
    ...
    logging.info(f"Iniciando integração de Movimentação de Estoque. Lendo de: {sql_database_source} | Gravando em: {sql_database_dest}")
    ...
    try:
        ...
        with pyodbc.connect(conn_str_source) as conn_source:
            ...
            # Busca todas as movimentações da origem
            query_select = "SELECT * FROM erp.estoque_movimentacao"
            cursor_source.execute(query_select)
            rows = cursor_source.fetchall()

            if not rows:
                logging.warning("Nenhum registro encontrado na origem (erp.estoque_movimentacao).")
                return
        ...
        with pyodbc.connect(conn_str_dest) as conn_dest:
            ...
            table_name = "dbo.estoque_movimentacao"
            ...
            logging.info(f"Carga finalizada: {len(rows)} movimentações inseridas com sucesso no destino.")          

    except pyodbc.Error as e:
        logging.error(f"Erro de SQL no processamento de estoque_movimentacao: {str(e)}")
        raise
    except Exception as e:
        logging.error(f"Erro inesperado no processo de estoque_movimentacao: {str(e)}")
        raise