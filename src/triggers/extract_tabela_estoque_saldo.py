...
# Mantive o agendamento para 1 vez por dia às 06:00, seguindo o padrão.
...
def extract_estoque_saldo(myTimer: func.TimerRequest) -> None:
    ...
    logging.info(f"Iniciando integração de Saldo de Estoque. Lendo de: {sql_database_source} | Gravando em: {sql_database_dest}")
    ...
    try:
        ...
        with pyodbc.connect(conn_str_source) as conn_source:
            ...
            # Busca todos os saldos de estoque da origem
            query_select = "SELECT * FROM erp.estoque_saldo"
            cursor_source.execute(query_select)
            rows = cursor_source.fetchall()

            if not rows:
                logging.warning("Nenhum registro encontrado na origem (erp.estoque_saldo).")
                return
        ...
        with pyodbc.connect(conn_str_dest) as conn_dest:
            ...
            logging.info(f"Carga finalizada: {len(rows)} saldos de estoque inseridos com sucesso no destino.")          

    except pyodbc.Error as e:
        logging.error(f"Erro de SQL no processamento de estoque_saldo: {str(e)}")
        raise
    except Exception as e:
        logging.error(f"Erro inesperado no processo de estoque_saldo: {str(e)}")
        raise