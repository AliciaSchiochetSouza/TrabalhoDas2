...
def extract_pedido_item(myTimer: func.TimerRequest) -> None:
    ...
    logging.info(f"Iniciando integração de Itens do Pedido. Lendo de: {sql_database_source} | Gravando em: {sql_database_dest}")
    ...
    try:
        ...
        with pyodbc.connect(conn_str_source) as conn_source:
            ...
            if not rows:
                logging.warning("Nenhum registro encontrado na origem (erp.pedido_item).")
                return
        ...
        with pyodbc.connect(conn_str_dest) as conn_dest:
            ...
            logging.info(f"Carga finalizada: {len(rows)} itens de pedido inseridos com sucesso no destino.")          

    except pyodbc.Error as e:
        logging.error(f"Erro de SQL no processamento de pedido_item: {str(e)}")
        raise
    except Exception as e:
        logging.error(f"Erro inesperado no processo de pedido_item: {str(e)}")
        raise