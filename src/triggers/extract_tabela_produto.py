...
def extract_produto(myTimer: func.TimerRequest) -> None:
    ...
    logging.info(f"Iniciando integração de Produtos. Lendo de: {sql_database_source} | Gravando em: {sql_database_dest}")
    ...
    try:
        ...
        with pyodbc.connect(conn_str_source) as conn_source:
            ...
            query_select = "SELECT * FROM erp.produto"
            cursor_source.execute(query_select)
            rows = cursor_source.fetchall()

            if not rows:
                logging.warning("Nenhum registro encontrado na origem (erp.produto).")
                return

            columns = [column[0] for column in cursor_source.description]
            logging.info(f"Extração bem-sucedida: {len(rows)} produtos encontrados.")
        ...
        with pyodbc.connect(conn_str_dest) as conn_dest:
            ...
            # Habilita IDENTITY_INSERT caso a tabela use chave sequencial
            cursor_dest.execute(f"SET IDENTITY_INSERT {table_name} ON")
            
            # Executa a carga em lote
            cursor_dest.executemany(insert_query, rows)
            
            # Desabilita IDENTITY_INSERT
            cursor_dest.execute(f"SET IDENTITY_INSERT {table_name} OFF")

            # Efetiva as transações
            conn_dest.commit()

            logging.info(f"Carga finalizada: {len(rows)} produtos inseridos com sucesso no destino.")          

    except pyodbc.Error as e:
        logging.error(f"Erro de SQL no processamento de produto: {str(e)}")
        raise
    except Exception as e:
        logging.error(f"Erro inesperado no processo de produto: {str(e)}")
        raise