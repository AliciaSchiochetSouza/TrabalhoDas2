...
def extract_representante(myTimer: func.TimerRequest) -> None:
    ...
    logging.info(f"Iniciando integração de Representante. Lendo de: {sql_database_source} | Gravando em: {sql_database_dest}")
    ...
    try:
        ...
        with pyodbc.connect(conn_str_source) as conn_source:
            ...
            # Busca todos os representantes da origem
            query_select = "SELECT * FROM erp.representante"
            cursor_source.execute(query_select)
            rows = cursor_source.fetchall()

            if not rows:
                logging.warning("Nenhum registro encontrado na origem (erp.representante).")
                return
        ...
        with pyodbc.connect(conn_str_dest) as conn_dest:
            ...
            logging.info(f"Carga finalizada: {len(rows)} representantes inseridos no destino.")          

    except pyodbc.Error as e:
        logging.error(f"Erro de SQL no processamento de representante: {str(e)}")
        raise
    except Exception as e:
        logging.error(f"Erro inesperado no processo de representante: {str(e)}")
        raise