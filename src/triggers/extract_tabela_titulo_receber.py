...
def extract_titulo_receber(myTimer: func.TimerRequest) -> None:
    ...
    logging.info(f"Iniciando integração de Títulos a Receber. Lendo de: {sql_database_source} | Gravando em: {sql_database_dest}")
    ...
    try:
        ...
        with pyodbc.connect(conn_str_source) as conn_source:
            ...
            # CORRIGIDO: Adicionado o ponto entre erp e titulo_receber
            query_select = "SELECT * FROM erp.titulo_receber"
            cursor_source.execute(query_select)
            rows = cursor_source.fetchall()

            if not rows:
                logging.warning("Nenhum registro encontrado na origem (erp.titulo_receber).")
                return

            columns = [column[0] for column in cursor_source.description]
            logging.info(f"Extração bem-sucedida: {len(rows)} títulos encontrados.")
        ...
        with pyodbc.connect(conn_str_dest) as conn_dest:
            ...
            logging.info(f"Carga finalizada: {len(rows)} títulos inseridos no destino.")          

    except pyodbc.Error as e:
        logging.error(f"Erro de SQL no processamento de titulo_receber: {str(e)}")
        raise
    except Exception as e:
        logging.error(f"Erro inesperado no processo de titulo_receber: {str(e)}")
        raise